# FG-CLIP2-So400m（1152 维）离线镜像构建与 HTTP 服务部署

> 适用环境：CentOS 7.6 / NVIDIA 驱动 515.76（CUDA 11.7）/ Docker 19.03 / Tesla T4  
> 目标：在 `10.0.0.157` 上构建 `fgclip2:cu117-1152` 镜像，提供 `8200` 端口图文向量 HTTP 服务  
...
POST /api/v1/embed/image  → 1152 维图片向量
    ↓
embed_test（Go 联调）/ go-server vl_bridge（VlSearch 召回）
```
...
### 3.3 wheels（transformers + HTTP 依赖）
...
#### 协议 B：go-server vl_bridge（VlSearch 召回）

```http
POST /api/v1/embed/text
Content-Type: application/json

{"texts":["找一个人"],"part":false,"project":false}
```

expand 模式（VlSearch 一次发多条 query 文本）：

```http
POST /api/v1/embed/text
Content-Type: application/json

{"texts":["A","B","C","D"],"part":false,"project":false}
```

期望响应：`count=4`，`features` 为 **4 个** 1152 维向量数组。
...
> **注意**：文本接口字段是 **`texts`（数组）** 不是 `text`；`texts` 长度必须与返回的 `features` 等长。`part` / `project` 为请求侧标志，服务端忽略即可。  
> 图片接口两种客户端相同：字段名必须是 **`image`**。
...
curl -s -X POST http://127.0.0.1:8200/api/v1/embed/text \
  -H "Content-Type: application/json" \
  -d '{"texts":["找一个人"],"part":false,"project":false}'

# expand 批量（4 条文本 → 4 个向量）
curl -s -X POST http://127.0.0.1:8200/api/v1/embed/text \
  -H "Content-Type: application/json" \
  -d '{"texts":["A","B","C","D"],"project":false}' | python3 -c "
import sys,json
d=json.load(sys.stdin)
print('ok=',d['ok'],'count=',d['count'],'dim=',d['dim'],'features_len=',len(d['features']))
assert d['count']==len(d['features'])==4, 'batch mismatch'
"
```
...
# 批量文本（expand 模式，8 条 → 8 个向量）
curl -s -X POST http://127.0.0.1:8200/api/v1/embed/text \
  -H "Content-Type: application/json" \
  -d '{"texts":["t1","t2","t3","t4","t5","t6","t7","t8"],"project":false}' | python3 -c "
import sys,json
d=json.load(sys.stdin)
assert d['ok'] and d['count']==8 and len(d['features'])==8
print('batch OK', d['count'], d['dim'])
"

# 看日志是否打出 batch 信息
docker logs --tail 20 fgclip2-gpu3
# 期望：embed/text batch=8 texts=[...]
#       embed/text done count=8 dim=1152
```
...
| 文件 | 说明 |
|------|------|
| **`FG-CLIP2-So400m-部署文档.md`** | 本文档（含完整 Dockerfile + embed_server.py） |

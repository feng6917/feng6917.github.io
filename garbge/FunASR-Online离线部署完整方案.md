...
11. **`docker restart funasr-online` 后不会自动拉起 2pass 进程**，需再执行一次第五节的「启动 FunASR 2pass 服务」命令，或直接 `./start_funasr.sh`。

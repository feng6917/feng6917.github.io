...
2. install.sh 调整
   ...
   ```bash
        ...
        # 调整为本地安装
        LOCAL_OLLAMA_ROCM_TGZ="./ollama-linux-amd64-rocm.tgz${VER_PARAM}"
        if [ -f "$LOCAL_OLLAMA_ROCM_TGZ" ]; then
            status "Installing from local file $LOCAL_OLLAMA_ROCM_TGZ"
            $SUDO tar -xzf "$LOCAL_OLLAMA_ROCM_TGZ" -C "$OLLAMA_INSTALL_DIR"
            install_success
            status "AMD GPU ready."
            exit 0
        else
            echo "Error: The local file $LOCAL_OLLAMA_ROCM_TGZ does not exist."
            exit 1
        fi

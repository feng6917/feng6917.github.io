...
2. Redissesion 具备watchdog 看门狗，定期检查线程是否过期，自动续命。集群使用lua脚本。(lua脚本中的命令是单线程执行)
...
1. 顺序读写：Kafka 使用顺序读写的方式，将数据写入磁盘。顺序读写比随机读写更快，因为磁盘的随机读写需要移动磁头，而顺序读写可以直接读取到数据。
2. 数据压缩：Kafka 支持对数据进行压缩，可以减少磁盘空间的使用和网络传输的数据量，从而提高性能。
...
1. 分词是指将一段文本中有用的词汇提取出来。
   ...
   - 爬虫+大数据+AI分析：根据语义分心（NLP），词频，上下文推测筛选
...
- OLTP:
    1. 联机事务处理系统，就是对数据的增删改查等操作
    2. 存储的是业务数据，来记录某类业务事件的发生，比如下单，支付，注册等
    3. 典型代表有MySQL、Oracle等数据库，对应的网站、系统应用后端数据库
    4. 针对事务进行操作，对响应时间要求高，面向前台应用的，应用比较简单，数据量相对较少，是GB级别的

- OLAP：
    1. 联机分析处理系统
    2. 存储的是历史数据，对应的风控平台、BI平台、数据可视化等系统
    3. OLAP是数据仓库系统的主要应用，支持复杂的分析操作，侧重决策，并且提供直观易懂的查询结果
    4. 典型代表有Hive, ClickHouse
    5. 针对基于查询的分析系统，基础数据来源于生产系统中的操作数据，数据量非常大，常规是TB级别的，面向群体：分析决策等人员
